...
def fetch_statsapi_for_date(date_str: str, out_dir: Path):
    ...
    for g in games:
        ...
        try:
            ...
            for play_sequence, play in enumerate(plays):
                # Use the actual field names from the JSON
                at_bat_index = play.get("about", {}).get("atBatIndex")
                
                # Since playIndex doesn't exist, create event_index using play sequence
                event_index = play_sequence
                
                # Skip only if at_bat_index is missing (it shouldn't be)
                ...
                row = {
                    ...
                    "event_index": event_index,  # Use play sequence number
