...
def fetch_roster_for_date(date_str: str, output_dir: str):
    ...
    for g in games:
        for team_id, side in ((g["home_id"], "home"), (g["away_id"], "away")):
            # Historical roster via the 'team_roster' endpoint
            try:
                data = statsapi.get(
                    "team_roster",
                    {"teamId": team_id, "date": date_str}
                )
            except Exception as e:
                print(f"❌ Roster error for team {team_id} on {date_str}: {e}")
                continue

            # drill into the payload: data["roster"]["roster"]
            recs = (
                data.get("roster", {})
                    .get("roster", [])
...
def main():
    p = argparse.ArgumentParser(
        description="Backfill Statcast, StatsAPI PBP & Rosters to Parquet."
    )
